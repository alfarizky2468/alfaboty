...
module.exports = HandleMsg = async (aruga, message) => {
	...
	// Filter Banned People
        ...
        case 'help':
            ...
            break
        case 'menuadmin':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup (ノ•̀ o •́ )ノ ~ ┻━┻!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup (ノ•̀ o •́ )ノ ~ ┻━┻!', id)
        ...
        case 'ownerbot':
            ...
            .then(() => aruga.sendText(from, 'Jika kalian ingin request fitur silahkan chat nomor owner (｡•̀ᴗ-)✧!'))
            break
        case 'join':
            ...
            if (!islink) return aruga.reply(from, 'Maaf link group-nya salah nih hyung (っ˘̩╭╮˘̩)っ! silahkan kirim link yang benar', id)
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await aruga.sendText(from, 'Berhasil join grup via link (｡•̀ᴗ-)✧')
                          await aruga.sendText(chekgrup.id, `Hai minna~, Im Alfa Bot◝(　ﾟ∀ ﾟ )◟. To find out the commands on this bot type ${prefix}menu`)
                      })
            } else {
                ...
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
                          await aruga.reply(from, 'Berhasil join grup via link (｡•̀ᴗ-)✧', id)
                      })
                      .catch(() => {
                          aruga.reply(from, 'Gagal(༎ຶ ෴ ༎ຶ)', id)
                      })
            }
            break
        case 'botstat': {
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                if (isMedia || isQuotedImage) {
                    ...
                    console.log(err)
	   	            await aruga.reply(from, 'Maaf batas penggunaan hari ini sudah mencapai maksimal ( ･ั﹏･ั)', id)
                    }
                }
            } else if (args.length === 1) {
                if (!isUrl(url)) { await aruga.reply(from, 'Maaf, link yang kamu kirim tidak valid ( ･ั﹏･ั)', id) }
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? aruga.sendText(from, 'Maaf, link yang kamu kirim tidak memuat gambar ( ･ั﹏･ั)')
                    : aruga.reply(from, 'Here\'s your sticker')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
                await aruga.reply(from, `Tidak ada gambar (ノ•̀ o •́ )ノ ~ ┻━┻ ! Untuk menggunakan ${prefix}sticker\n\n\nKirim gambar dengan caption\n${prefix}sticker <biasa>\n${prefix}sticker nobg <tanpa background>\n\natau Kirim pesan dengan\n${prefix}sticker <link_gambar>`, id)
            }
            break
        case 'stickergif':
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    aruga.reply(from, '[WAIT] Sedang di proses nih hyung⏳ silahkan tunggu ± 1 min (｡•̀ᴗ-)✧!', id)
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            aruga.reply(from, 'Maaf filenya terlalu besar (ノ•̀ o •́ )ノ ~ ┻━┻!', id)
                        })
                    })
                  } else {
        ...
        case 'stikergiphy':
        case 'stickergiphy':
            if (args.length !== 1) return aruga.reply(from, `Maaf, format pesan salah nih hyung!.\nKetik pesan dengan ${prefix}stickergiphy <link_giphy>`, id)
            ...
            if (isGiphy) {
                ...
                .catch(() => {
                    aruga.reply(from, `Ada yang error (ノ•̀ o •́ )ノ ~ ┻━┻!`, id)
                })
            } else {
        ...
        case 'daerah':
            ...
            .catch(() => {
                aruga.reply(from, 'Ada yang Error nih hyung .·´¯`(>▂<)´¯`·.', id)
            })
            break
        //Media
        case 'ytmp3':
		...
		case 'facebook':
			...
			.then(async (res) => {
				...
				await aruga.sendFileFromUrl(from, linkhd, '', 'Nih ngab videonya (づ｡◕‿‿◕｡)づ', id)
				.catch(async () => {
					await aruga.sendFileFromUrl(from, linksd, '', 'Nih ngab videonya (づ｡◕‿‿◕｡)づ', id)
					.catch(() => {
						aruga.reply(from, 'Maaf url anda tidak dapat ditemukan ( ꈨຶ ˙̫̮ ꈨຶ )', id)
					})
				})
			})
			break
		...
		//Primbon Menu
		case 'artinama':
			if (args.length == 0) return aruga.reply(from, `(☞ﾟヮﾟ)☞ Untuk mengetahui arti nama seseorang\nketik ${prefix}artinama namakamu`, id)
        ...
        case 'anime':
            ...
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error nih .·´¯`(>▂<)´¯`·.', id)
                })
            } else {
        ...
        case 'images':
            ...
            .catch(() => {
                aruga.reply(from, 'Ada yang Error nih say!', id)
            })
            break
        case 'sreddit':
            ...
            .catch(() => {
                aruga.reply(from, 'Ada yang Error nih kak!', id)
            })
	    break
        case 'resep':
        ...
        case 'whatanime':
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                aruga.reply(from, 'ditunggu yaa,lagi di proses nih hyung (｡•̀ᴗ-)✧', id)
        ...
        case 'klasemen':
		case 'klasmen':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf say, perintah ini hanya dapat dipakai didalam grup (｡•̀ᴗ-)✧', id)
	    ...
	    case 'add':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf say, perintah ini hanya dapat dipakai didalam grup yaa (｡•̀ᴗ-)✧', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal nih hehe, perintah ini hanya dapat digunakan oleh admin grup (｡•̀ᴗ-)✧', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal hyung, silahkan tambahkan bot sebagai admin grup (｡•̀ᴗ-)✧', id)
	        if (args.length !== 1) return aruga.reply(from, `Untuk menggunakan ${prefix}add\nPenggunaan: ${prefix}add <nomor>\ncontoh: ${prefix}add 628xxx`, id)
                try {
                    ...
                    aruga.reply(from, 'Tidak dapat menambahkan target nih', id)
                }
            break
        case 'kick':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf hyung, perintah ini hanya dapat dipakai didalam grup yaa (｡•̀ᴗ-)✧', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal nih hyung, perintah ini hanya dapat digunakan oleh admin grup (｡•̀ᴗ-)✧', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal say, silahkan tambahkan bot sebagai admin grup (｡•̀ᴗ-)✧', id)
            ...
            break
        case 'promote':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf hyung, perintah ini hanya dapat dipakai didalam grup (｡•̀ᴗ-)✧', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal nih hyung, perintah ini hanya dapat digunakan oleh admin grup (｡•̀ᴗ-)✧', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal nih hyung, silahkan tambahkan bot sebagai admin grup (｡•̀ᴗ-)✧', id)
            if (mentionedJidList.length !== 1) return aruga.reply(from, 'Maaf, hanya bisa mempromote 1 user', id)
            if (groupAdmins.includes(mentionedJidList[0])) return await aruga.reply(from, 'Maaf, user tersebut sudah menjadi admin (｡•̀ᴗ-)✧', id)
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Maaf hyung, format pesan salah.\nTidak dapat mempromote akun bot sendiri', id)
            await aruga.promoteParticipant(groupId, mentionedJidList[0])
            await aruga.sendTextWithMentions(from, `Request diterima (｡•̀ᴗ-)✧, menambahkan @${mentionedJidList[0].replace('@c.us', '')} sebagai admin.`)
            break
        case 'demote':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf hyung, perintah ini hanya dapat dipakai didalam grup (´-﹏-`；)', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal nih kak, perintah ini hanya dapat digunakan oleh admin grup (ノ•̀ o •́ )ノ ~ ┻━┻', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal nih kak, silahkan tambahkan bot sebagai admin grup (ノ•̀ o •́ )ノ ~ ┻━┻', id)
        ...
        case 'bye':
            ...
            break
        case 'del':
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal hyung, perintah ini hanya dapat digunakan oleh admin grup (´-﹏-`；)', id)
            if (!quotedMsg) return aruga.reply(from, `Maaf yaa, format pesan salah silahkan.\nReply pesan bot dengan caption ${prefix}del`, id)
        ...
        case 'everyone':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf hyung, perintah ini hanya dapat dipakai didalam grup (´-﹏-`；)', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal nih kak, perintah ini hanya dapat digunakan oleh admin grup (´-﹏-`；)', id)
            ...
            break
		case 'simisimi':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf hyung, perintah ini hanya dapat dipakai didalam grup 人 •͈ᴗ•͈)', id)
			aruga.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			break
		case 'simi':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf hyung, perintah ini hanya dapat dipakai didalam grup 人 •͈ᴗ•͈)', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal nih hyung, perintah ini hanya dapat digunakan oleh admin grup 人 •͈ᴗ•͈)', id)
			...
			if (args[0] == 'on') {
                ...
                aruga.reply(from, 'Mengaktifkan bot simi-simi (｡•̀ᴗ-)✧', id)
			} else if (args[0] == 'off') {
				let inxx = simi.indexOf(chatId)
				simi.splice(inxx, 1)
				fs.writeFileSync('./settings/simi.json', JSON.stringify(simi))
				aruga.reply(from, 'Menonaktifkan bot simi-simi (｡•̀ᴗ-)✧', id)
			} else {
				aruga.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			}
			break
		case 'katakasar':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup ( ꈨຶ ˙̫̮ ꈨຶ )', id)
		...
		case 'kasar':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup ( ꈨຶ ˙̫̮ ꈨຶ )', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup ( ꈨຶ ˙̫̮ ꈨຶ )', id)
			...
			if (args[0] == 'on') {
				...
				aruga.reply(from, 'Fitur Anti Kasar sudah di Aktifkan ヽ(｡◕o◕｡)ﾉ', id)
			} else if (args[0] == 'off') {
				let nixx = ngegas.indexOf(chatId)
				ngegas.splice(nixx, 1)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				aruga.reply(from, 'Fitur Anti Kasar sudah di non-Aktifkan ヽ(｡◕o◕｡)ﾉ', id)
			} else {
				aruga.reply(from, `Untuk mengaktifkan Fitur Kata Kasar pada Group Chat\n\nApasih kegunaan Fitur Ini? Apabila seseorang mengucapkan kata kasar akan mendapatkan denda\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			}
			break
		...
		case 'mutegrup':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf hyung, perintah ini hanya dapat dipakai didalam grup (｡•̀ᴗ-)✧', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal nih hyung, perintah ini hanya dapat digunakan oleh admin grup (｡•̀ᴗ-)✧', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup (ノ•̀ o •́ )ノ ~ ┻━┻', id)
            ...
            if (args[0] == 'on') {
				aruga.setGroupToAdminsOnly(groupId, true).then(() => aruga.sendText(from, 'Berhasil mengubah agar hanya admin yang dapat chat (｡•̀ᴗ-)✧'))
			} else if (args[0] == 'off') {
				aruga.setGroupToAdminsOnly(groupId, false).then(() => aruga.sendText(from, 'Berhasil mengubah agar semua anggota dapat chat (｡•̀ᴗ-)✧'))
			} else {
				aruga.reply(from, `Untuk mengubah settingan group chat agar hanya admin saja yang bisa chat\n\nPenggunaan:\n${prefix}mutegrup on --aktifkan\n${prefix}mutegrup off --nonaktifkan`, id)
			}
			break
		case 'setprofile':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf hyung, perintah ini hanya dapat dipakai didalam grup (ノ•̀ o •́ )ノ ~ ┻━┻!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal nih hyung, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup (｡•̀ᴗ-)✧', id)
			if (isMedia && type == 'image' || isQuotedImage) {
				...
				if (!isUrl(url)) { await aruga.reply(from, 'Maaf, link yang kamu kirim tidak valid nih hyung (ノ•̀ o •́ )ノ ~ ┻━┻', id) }
				aruga.setGroupIconByUrl(groupId, url).then((r) => (!r && r !== undefined)
				? aruga.reply(from, 'Maaf, link yang kamu kirim tidak memuat gambar nih hyung (;ŏ﹏ŏ)', id)
				: aruga.reply(from, 'Berhasil mengubah profile group ヽ(｡◕o◕｡)ﾉ', id))
			} else {
        ...
        if (!isOwner) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai oleh owner grup (ノ•̀ o •́ )ノ ~ ┻━┻!', id)
        if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup (ノ•̀ o •́ )ノ ~ ┻━┻!', id)
        ...
        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return aruga.reply(from, 'Perintah ini hanya untuk Owner bot ya hyung (｡•̀ᴗ-)✧', id)
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *A L F A  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *A L F A  B C* 〙\n\n${msg}`)
            }
        ...
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            ...
            for (let gclist of allGroupz) {
                await aruga.sendText(gclist.contact.id, `Maaf ya hyung aku lagi dibersihin nih (っ˘̩╭╮˘̩)っ, total chat aktif : ${allChatz.length}`)
                await aruga.leaveGroup(gclist.contact.id)
                await aruga.deleteChat(gclist.contact.id)
            }
            aruga.reply(from, 'Success leave all group (｡•̀ᴗ-)✧', id)
            break
        case 'clearall': //menghapus seluruh pesan diakun bot
            ...
            aruga.reply(from, 'Success clear all chat (｡•̀ᴗ-)✧', id)
            break
        default:
            break
        }
